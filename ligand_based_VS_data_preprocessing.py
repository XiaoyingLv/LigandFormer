...
from util.scaffold_overlap import scaffold_summary
...
def balance_resample(dataset, class_labels, class_num=None, expected_class_distribution=None, expected_sample_num=None,
                     shuffled=True):
    """
...
def dataset_partition_for_regress(dataset, partition_ratio=(0.8, 0.2)):  # todo: add support for type(dataset) = dict
    """
    Partition dataset for regression task
    :param dataset: list of data items (np.ndarray) to be partitioned, each array should have the 1st dimension for the sample number
    :param partition_ratio: list of float, required to sum to 1.0
    :return: list of datasets corresponding to `partition_ratio`
    """
    dataset_partitions = []
    dataset_partition_train = []
    dataset_partition_test = []
    for item in dataset:
        if item is None:
            dataset_partition_train.append(None)
            dataset_partition_test.append(None)
        elif len(item.shape) > 1:
            data_train = item[:int(partition_ratio[0] * len(item)), ::]
            dataset_partition_train.append(data_train)
            data_test = item[int(partition_ratio[0] * len(item)):, ::]
            dataset_partition_test.append(data_test)
        else:
            data_train = item[:int(partition_ratio[0] * len(item))]
            dataset_partition_train.append(data_train)
            data_test = item[int(partition_ratio[0] * len(item)):]
            dataset_partition_test.append(data_test)
    dataset_partitions.append(dataset_partition_train)
    dataset_partitions.append(dataset_partition_test)
...
def calc_data_distribution(class_labels, class_num=None):
    ...
    for i in range(class_num + 1):
        if i == 0:
            ...
            print('label range: <= %d, sample_number %d, total_sample_number %d' % (
            i, (class_labels <= i).sum(), sample_num))
        elif i < class_num:
            data_distribution[i] = ((class_labels <= i).sum() - (class_labels <= i - 1).sum()) / sample_num
            print('label range: (%d, %d], sample_number %d, total_sample_number %d' % (
            i - 1, i, (class_labels <= i).sum() - (class_labels <= i - 1).sum(), sample_num))
        else:
            data_distribution[i] = (class_labels > i - 1).sum() / sample_num
            print('label range: (%d, %d], sample_number %d, total_sample_number %d' % (
            i - 1, i, (class_labels > i - 1).sum(), sample_num))
...
def compute_scaffold(smiles_string, include_chirality=False):
    """
    Compute the Bemis-Murcko scaffold for a SMILES string
    :return SMILES of the BM scaffold
    """
...
def get_scaffold_group(SMILES_list, include_chirality=True):
    """
    This function will group SMILES samples by scaffold meanwhile maintain their relative order within each group
    The group order is determined by the first sample's index in the original `SMILES_list`
    :param SMILES_list: list of SMILES strings
    :param include_chirality:
    :return: index reordered and scaffold_dict
    """
    if sys.version_info.minor < 6:  # python doesn't has ordered set yet, use ordered dict instead
        from collections import OrderedDict
        scaffold_dict = OrderedDict()
    else:
        scaffold_dict = dict()  # dict is ordered since python 3.6
    for idx, smiles_string in enumerate(SMILES_list):
        try:
            scaffold = compute_scaffold(smiles_string, include_chirality=include_chirality)
        except:
            continue
        if scaffold not in scaffold_dict:
            scaffold_dict[scaffold] = [idx]
        else:
            scaffold_dict[scaffold].append(idx)
    idxs = []
    for scaffold in scaffold_dict:
        idxs.extend(scaffold_dict[scaffold])
    return idxs, scaffold_dict
...
def convert_SMILES_to_graph(SMILES=None, canonical_mol=None, atom_dict=None, version='G0'):
    ...
    if version == 'G0':
        ...
        if canonical_mol is not None:
            ...
            molecule = canonicalize_molecule(molecule, addH=False)
...
def data_preprocess(args):
    ...
    group_by_scaffold = args.group_by_scaffold.lower() in {'true', 'yes'}
    feature_ver = args.feature_ver.lower()
    atom_multifeatures = args.atom_multifeatures.lower() in {'true', 'yes'}
    ...
    if atom_multifeatures:
        print('Using multi features for each atom: 1 H number; 2 Degree; 3 Formal charge;  4 Chirality label; 5 Hybridization; 6 Aromaticity; 7 Atomic Number')
    ...
    if args.doc is None:
        featurized_file = os.path.join(save_folder,
                                       basename_without_ext + '_%s_%s@%s.gpkl' % (task, feature_ver, time_stamp,))
        stdout_log_file = os.path.join(save_folder,
                                       basename_without_ext + '_%s_preprocessing_stdout@%s.txt' % (task, time_stamp,))
        stderr_log_file = os.path.join(save_folder,
                                       basename_without_ext + '_%s_preprocessing_stderr@%s.txt' % (task, time_stamp,))
    else:
        featurized_file = os.path.join(save_folder, basename_without_ext + '_%s_%s_%s@%s.gpkl' % (
        args.doc, task, feature_ver, time_stamp,))
        stdout_log_file = os.path.join(save_folder, basename_without_ext + '_%s_%s_preprocessing_stdout@%s.txt' % (
        args.doc, task, time_stamp,))
        stderr_log_file = os.path.join(save_folder, basename_without_ext + '_%s_%s_preprocessing_stderr@%s.txt' % (
        args.doc, task, time_stamp,))
    ...
    if label_col_name in data_frame:
        ...
        if task not in {'classification', 'regression'}:
            raise ValueError('The task "%s" not found, please check' % task)
    ...
    if labels is not None:
        if task == 'classification':
            ...
            label_min, label_max = labels.min(), labels.max()
            print('\nlabel_min = %0.01f, label_max = %0.01f' % (label_min, label_max))
            label_min = label_min
            label_max = label_max
            data_section = label_max - label_min
            # labels -= label_min
    ...
    for i in range(sample_num):
        ...
        if molecule is None or len(molecule.GetBonds()) <= 0:
            ...
            if feature_ver == '0':
                ...
                for i, each_xs in enumerate(xs):
                    atom_feats[i].append(each_xs)
                if len(atom_feats) > 1:
                    mol_feature = (atom_feats, es)
                else:
                    mol_feature = (xs, es)
            else:
    ...
    # --- scaffold analysis ---#
    scaffold_dict = None  # for prediction dataset, scaffold analysis is disabled
    if task in {'classification', 'regression'}:
        print('Do scaffold analysis ...')
        idxs, scaffold_dict = get_scaffold_group(SMILES_list=SMILES_list, include_chirality=use_chirality)
        print('Total %d scaffold, %d samples' % (len(scaffold_dict), len(idxs)))
        print(scaffold_summary(scaffold_dict))
        # for i, scaffold in enumerate(scaffold_dict.keys()):
        # print('scaffold[%d], #sample = %d, description = %s' % (i, len(scaffold_dict[scaffold]), scaffold))
    ...
    dataset['scaffold_dict'] = scaffold_dict
    ...
    if task in {'classification', 'regression'}:
        if 1.0 > partition_ratio > 0.0:
            ...
            if not group_by_scaffold:
                ...
                dataset_shuffled = dataset_shuffle((SMILES_list, features, labels, IDs, sample_weights), index)
            ...
            trainset_file = os.path.join(save_folder, '%s_%s_%s_trainset_%d@%s.gpkl' % (
            basename_without_ext, task, feature_ver, trainset_sample_num, time_stamp))
            testset_file = os.path.join(save_folder, '%s_%s_%s_testset_%d@%s.gpkl' % (
            basename_without_ext, task, feature_ver, testset_sample_num, time_stamp))
...
if __name__ == "__main__":
    ...
    argparser.add_argument('-smiles_col_name', default='Cleaned_SMILES', type=str,
                           help='column name for SMILES, required')
    ...
    argparser.add_argument('-sample_weight_col_name', default='sample weight', type=str,
                           help='column name for sample weight, optional')
    argparser.add_argument('-task', default='classification', type=str, help='classification/regression/prediction')

    argparser.add_argument('-feature_ver', default='2', type=str,
                           help='{1|2|0}, different featurization method, 1 = kekulized smiles, 2 = graph, 0 = deepchem fingerprint')
    argparser.add_argument('-atom_multifeatures', default='true', type=str,
                        help='whether to multi-features for each atom')
    argparser.add_argument('-use_chirality', default='true', type=str,
                           help='whether consider chirality for feature ver 0 and scaffold analysis')
    argparser.add_argument('-addH', default='False', type=str,
                           help='whether add back hydrogen atoms when canonicalize molecule')

    argparser.add_argument('-partition_ratio', default=0.2, type=float,
                           help='percent of samples for testing, any value <= 0 or >= 1 will disable the dataset splitting procedure')
    argparser.add_argument('-group_by_scaffold', default='false', type=str, help='whether turn on `split by scaffold`')
